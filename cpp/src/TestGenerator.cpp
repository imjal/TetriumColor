...
)
{
    ...
    // Call Python NewTestRYGB method
...
)
{
    ...
    // Convert ColorTestResult to Python enum
    PyObject* pColorTestResultModule = PyImport_ImportModule("TetriumColor.Utils.CustomTypes");
    if (!pColorTestResultModule) {
        Py_DECREF(pMethod);
        PyErr_Print();
        throw std::runtime_error("TestGenerator: Failed to import CustomTypes module");
    }

    PyObject* pColorTestResultType
        = PyObject_GetAttrString(pColorTestResultModule, "ColorTestResult");
    Py_DECREF(pColorTestResultModule);

    if (!pColorTestResultType) {
        Py_DECREF(pMethod);
        PyErr_Print();
        throw std::runtime_error("TestGenerator: Failed to get ColorTestResult type");
    }

    const char* result_name = (previous_result == ColorTestResult::Success) ? "Success" : "Failure";
    PyObject* pPreviousResult = PyObject_GetAttrString(pColorTestResultType, result_name);
    Py_DECREF(pColorTestResultType);
    ...
    // Check if result is None (test complete)
    if (pResult == Py_None) {
        Py_DECREF(pResult);
        return std::nullopt;
    }
