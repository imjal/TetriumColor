...
ansari_path = "../../data/inksets/xp15000/cmyr-inks.csv"
...
fp_df["Name"] = fp_df["Name"].str.rstrip()
...
print(filtered_fp_df['Name'])
pdb.set_trace()

# Also include the paper data from fp_df
paper_df = fp_df[fp_df['Name'] == 'paper'].copy()
...
# Set the Index for paper_df to be after the filtered inks
paper_df['Index'] = len(ansari_df) + len(filtered_fp_df)

# Concatenate ansari_df, filtered_fp_df, and paper_df
filtered_combined_df = pd.concat([ansari_df, filtered_fp_df, paper_df], ignore_index=True)

# Save to a new CSV
# filtered_combined_no_index = filtered_combined_df.drop(columns=['Index'])
...
filtered_combined_df['clogged'] = 0
# Save again with the clogged column
filtered_combined_df.to_csv(filtered_output_no_index_csv_path, index=False)
