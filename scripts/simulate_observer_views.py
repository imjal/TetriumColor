...
trichromat observer genotypes, showing how different genetic variants of color vision
...
def load_disp_6p_image(base_filename: str) -> np.ndarray:
    """
    Load a DISP_6P image from RGB and OCV files.
...
def disp_6p_to_4channel(weights_6channel: np.ndarray,
                        led_mapping: List[int] = [0, 1, 3, 2, 1, 3]) -> np.ndarray:
    """
    Convert 6-channel display weights to 4-channel DISP weights.

    The 6-channel format represents alternating frames (RGB-OCV pattern).
    We average them to get 4-channel weights.

    Args:
        weights_6channel: Display weights (N, 6) in [0, 1]
        led_mapping: How 6 channels map to 4 LEDs (default: [0,1,3,2,1,3] for RGBO)

    Returns:
        Array of shape (N, 4) with 4-channel weights
    """
    n_pixels = weights_6channel.shape[0]
    weights_4channel = np.zeros((n_pixels, 4))
    ...
    for led_idx in range(4):
        ...
        if len(indices) > 0:
            weights_4channel[:, led_idx] = np.mean(weights_6channel[:, indices], axis=1)

    return weights_4channel


def compute_spectral_radiances(weights_4channel: np.ndarray,
                               primaries: List[Spectra]) -> np.ndarray:
    """
    Compute spectral radiances from display weights (vectorized).

    Args:
        weights_4channel: Display weights (N, 4) in [0, 1]
        primaries: List of 4 Spectra objects for RGBO primaries

    Returns:
        Array of shape (N, num_wavelengths) with spectral radiances
    """
    # Stack primary spectra into a matrix: (4, num_wavelengths)
    primary_matrix = np.array([p.data for p in primaries])

    # Compute radiances: (N, 4) @ (4, num_wavelengths) = (N, num_wavelengths)
    radiances = weights_4channel @ primary_matrix
...
def convert_disp_6p_to_srgb_for_observer(disp_6p_image: np.ndarray,
    ...
    The process is:
    1. Convert DISP_6P weights → 4-channel DISP weights (averaging frames)
    ...
    Args:
        ...
        display_primaries: List of 4 Spectra objects (RGBO primaries)
        led_mapping: How 6 channels map to 4 LEDs
    ...
    # Convert 6-channel to 4-channel (fast, no observer needed)
    pixels_4channel = disp_6p_to_4channel(pixels_6p, led_mapping)

    # Compute spectral radiances (vectorized matrix multiply)
    radiances = compute_spectral_radiances(pixels_4channel, display_primaries)
    ...
    if scaling_factor == 1.0:
        ...
        white_4channel = np.ones((1, 4))
        white_radiance = compute_spectral_radiances(white_4channel, display_primaries)
...
And create sRGB versions for 8 different trichromat observers.
        """
    )
    ...
    parser.add_argument('--num-observers', type=int, default=8,
                        help='Number of most common trichromat observers to simulate (default: 8)')
    ...
    parser.add_argument('--led-mapping', type=str, default='0,1,3,2,1,3',
                        help='LED mapping for 6P display as comma-separated values (default: 0,1,3,2,1,3)')
    ...
    # Load DISP_6P image
    print(f"\nLoading DISP_6P image from: {args.input_base}")
    disp_6p_image = load_disp_6p_image(args.input_base)
    ...
    # Initialize observer genotypes (only trichromats, dimension=3)
    print(f"\nInitializing observer genotypes (sex: {args.sex})...")
    observer_gen = ObserverGenotypes(wavelengths=wavelengths, dimensions=[2])

    # Get the most common trichromat genotypes
    ...
    print(f"\nSimulating views for top {len(genotypes)} trichromat genotypes:")
